...
public interface QuestionRepository extends JpaRepository<Question, Long> {
    ...
    @Query("SELECT COUNT(q) FROM Question q WHERE q.category.id = :categoryId AND q.game.id = :gameId AND q.status.statusName = com.brandonkamga.tekizz.gaming.qcm.domain.model.vo.QuestionStatusType.ACTIVE")
    ...
    @Query("SELECT q FROM Question q WHERE q.submittedBy IS NOT NULL AND q.status.statusName = com.brandonkamga.tekizz.gaming.qcm.domain.model.vo.QuestionStatusType.REVIEW")
    List<Question> findPendingContributions();

    @Query("SELECT COUNT(q) FROM Question q WHERE q.submittedBy IS NOT NULL AND q.status.statusName = com.brandonkamga.tekizz.gaming.qcm.domain.model.vo.QuestionStatusType.REVIEW")
