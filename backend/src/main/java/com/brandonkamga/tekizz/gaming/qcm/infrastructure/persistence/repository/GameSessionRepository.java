...
import com.brandonkamga.tekizz.gaming.qcm.infrastructure.persistence.entity.GameSession;
...
public interface GameSessionRepository extends JpaRepository<GameSession, Long> {
    ...
    @Query("SELECT COUNT(gs) FROM GameSession gs WHERE gs.user.id = :userId AND gs.status.statusName = com.brandonkamga.tekizz.gaming.qcm.domain.model.vo.GameStatusType.COMPLETED")
    Long countCompletedByUserId(@Param("userId") Long userId);

    @Query("SELECT SUM(gs.totalScore) FROM GameSession gs WHERE gs.user.id = :userId AND gs.status.statusName = com.brandonkamga.tekizz.gaming.qcm.domain.model.vo.GameStatusType.COMPLETED")
