...
public interface GameSessionRepository extends JpaRepository<GameSession, Long> {
    ...
    @Query("SELECT COUNT(gs) FROM GameSession gs WHERE gs.user.id = :userId AND gs.status.statusName = com.brandonkamga.tekizz.domain.GameStatusType.COMPLETED")
    Long countCompletedByUserId(@Param("userId") Long userId);
    
    @Query("SELECT SUM(gs.totalScore) FROM GameSession gs WHERE gs.user.id = :userId AND gs.status.statusName = com.brandonkamga.tekizz.domain.GameStatusType.COMPLETED")
